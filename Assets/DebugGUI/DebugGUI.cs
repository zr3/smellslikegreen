...
public class DebugGUI : MonoBehaviour
{
                ...
                if (mb != null && attributeKeys.ContainsKey(mb))
                {
                    ...
                    foreach (var key in attributeKeys[mb])
                    {
                        if (key.memberInfo is FieldInfo)
                        {
                            ...
                            float[] valSet = fieldInfo.GetValue(mb) as float[];
                            if (val != null)
                                graphDictionary[key].Push(val.Value);
                            else if (valSet != null)
                                graphDictionary[key].PushSet(valSet);
                        }
                        else if (key.memberInfo is PropertyInfo)
                        {
                            ...
                            float[] valSet = propertyInfo.GetValue(mb) as float[];
                            if (val != null)
                                graphDictionary[key].Push(val.Value);
                            else if (valSet != null)
                                graphDictionary[key].PushSet(valSet);
            ...
            // Fields
            {
                ...
                for (int i = 0; i < objectFields.Length; i++)
                {
                    ...
                    if (graphAttribute != null)
                    {
                        ...
                        if (objectFields[i].GetValue(mb) as float? == null && objectFields[i].GetValue(mb) as float[] == null)
                        {
                            Debug.LogError(string.Format("Cannot cast {0}.{1} to float or float[]. This member will be ignored.", monoType.Name, objectFields[i].Name));
                            continue;
                        }
    ...
    class GraphContainer
    {
        ...
        // Set the entirety of the graph
        public void PushSet(float[] vals)
        {
            static Func<int, int> LinearConversion(float a, float b)
            {
                float sourceValsPerTarget = a / b;
                return (int i) => Mathf.FloorToInt(i * sourceValsPerTarget);
            }
            static Func<int, int> LogConversion(float a, float b)
            {
                return (int i) => Mathf.FloorToInt(Mathf.Pow(10, i / 100f));
            }
            float valMax = Single.MinValue;
            float valMin = Single.MaxValue;
            var convert = LinearConversion(vals.Length, values.Length);
            //var convert = LogConversion(vals.Length, values.Length);
            for (int i = 0; i < values.Length; i++) {
                float val = vals[convert(i)];
                if (val > valMax) valMax = val;
                if (val < valMin) valMin = val;
                values[i] = val;
            }
            if (autoScale && (valMax > max || valMin < min))
            {
                SetMinMax(Mathf.Min(min, valMin), Mathf.Max(max, valMax));
            }
            ...
            // clear target
            target.SetPixels32(clearColorArray);
            ...
            for (int currentIndex = 0; currentIndex < values.Length - 1; currentIndex++) {
                ...
                DrawLine(target, currentIndex, y0, currentIndex + 1, y1, color);
            }
        }
